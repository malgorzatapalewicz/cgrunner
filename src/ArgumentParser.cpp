...
namespace ArgumentParser {
    ...
    bool isLimitParsable(const char *arg, T &limitValue, const char *limitType) {
        if (!isNonNegativeLimit(arg, limitType)) return false;
    ...
    bool parseCpuLimit(const char* arg, bool& cpuLimitProvided, int& cpuPercent) {
        cpuLimitProvided = isLimitParsable(arg, cpuPercent, "CPU");
        if (!cpuLimitProvided) {
            return false;
        }
        return true;
    ...
    bool parseMemoryLimit(const char* arg, bool& memoryLimitProvided, size_t& memoryBytes) {
        memoryLimitProvided = isLimitParsable(arg, memoryBytes, "Memory");
    ...
    bool processOption(int opt, const char* optarg, bool& cpuLimitProvided, bool& memoryLimitProvided, int& cpuPercent, size_t& memoryBytes) {
        switch (opt) {
            case 'c':
    ...
    void parseArguments(int argc, char* argv[], bool& cpuLimitProvided, bool& memoryLimitProvided, int& cpuPercent, size_t& memoryBytes) {
        int opt;
